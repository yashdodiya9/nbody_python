...
/*static float sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrang*sqrang) 
...
#define SQR(a) ((a) * (a))
#define CUBE(a) ((a) * (a) * (a))   //I've replace 123-128 with new code (130-132) as per perplexity 
...
/* Notes: Units are Mpc, not h^-1 Mpc. */
{
    ...
    (void)q_eff; // I've added "(void)var_name" as per perplexity
    
    float T_c_f, T_c, s_tilde, T_b_T0, T_b, f_baryon, T_full;

    float T_0_L0, T_0_C0, T_0, gamma_eff;
    (void)gamma_eff; (void)T_0_C0; (void)T_0_L0; (void)T_0; // I've added "(void)var_name" as per perplexity
    
    float T_nowiggles_L0, T_nowiggles_C0, T_nowiggles;
    (void)T_nowiggles_L0; (void)T_nowiggles_C0; (void)T_nowiggles; // I've added "(void)var_name" as per perplexity
