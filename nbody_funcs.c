...
void delta_fill(long* seed)
{
  ...
  long index1,index2;
  float amp,phasev,DD; 
    (void)phasev; //I've added (void)var_name as per perplexity
    (void)DD;     //I've added (void)var_name as per perplexity
  fftwf_complex *A;
...
void calpow(int f_flag, int Nbin, double* power, double* powerk, double* kmode, long *no)
{
  ...
  long index,index1,index2,c1=0;
    (void)fac;  //I've added (void)var_name as per perplexity
    (void)c1;   //I've added (void)var_name as per perplexity
...
void Zel_move_gradphi(float av,float **rra,float **vva)
{
  ...
  (void)DD; //I've added (void)var_name as per perplexity
  float vfac; // converts to peculiar velocity
...
int write_multiout(char *fname,long int seed,int output_flag,float **rra,float **vva,float vaa)
{
  ...
  if(output_flag!=1) 
    ...
    return 0; //I've used "return 0" as per perplexity
} 
/*-------------------------------------------------------------------------------------------------------*/
...
int read_output(char *fname, int read_flag,long int *seed,int *output_flag,int *in_flag,float **rra,float **vva,float *aa)
{
  ...
  int kk, dummy; 
    (void)kk; //I've added (void)var_name as per perplexity
  // header reading
