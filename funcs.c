...
#define JMAX 20
#include <stddef.h> //included "#include <stddef.h>" as per perplexity
...
{
	float trapzd(float (*func)(float), float a, float b, int n);
	int j=0, (void)j; //added "int j=0, (void)j;" in place of "int j;" as per perplexity
    float s=0.0, olds=0.0; //added this whole line as per perplexity 
	(void)s, (void)olds; //added "(void)var_name" as per perplexity in place of "float var_name"
	float dummy=0.0; // have added this whole line as per perplexity
	(void)dummy; //I've used "(void)var_name" & removed "float var_name" as per perplexity 
...
*/    //REPLACED qtrap() FUNCTION WITH NEW ONE AS PER PERPLEXITY
...
float qtrap(float (*func)(float), float a, float b)
{
    ...
    (void)dummy;  // Only if you don't actually use dummy
